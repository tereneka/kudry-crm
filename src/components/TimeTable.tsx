...
import { masters, registrations, Service, services } from '../data'
...
export default function TimeTable({ date }: Props) {
    const [regData, setRegData] = useState(registrations);
    const [cellData, setCellData] = useState<{ master: number, date: string, time: number }>()
    const [isRegModalOpen, setIsRegModalOpen] = useState(false);

    function handleCellClick(e: React.MouseEvent<HTMLTableDataCellElement, MouseEvent>) {
        if (e.currentTarget.dataset.master && e.currentTarget.dataset.time) {
            setCellData({
    ...
    const mastersList = masters.map(master => {
        return (
            <th className='time-table__time-header time-table__time-header_position_horizontal' key={master.id}>
                {master.name}
            </th>
        )
    });
    ...
    const timesList = timeArr.map(time => {
        return (
            <tr key={nanoid()} className='time-table__row'>
                <th
                    className='time-table__time-header time-table__time-header_position_vertical'
                >
                    {time}:00
                </th>
                {masters.map((master) => {
    ...
    const regList = regData
        ...
        .map(reg => {
            ...
            reg.serviceId.forEach(id => {
                const regService = services.find(service => service.id === id);
                if (regService) {
            ...
            const regMaster = masters.find(master => master.id === reg.masterId);
            if (regMaster) {
                leftMultiplier = masters.indexOf(regMaster)
            }
            ...
            return (
                ...
                >
                    ...
                    <a className='reg-card__link' href={`tel:${reg.phone}`}>
                        <img src={phone} alt="" />
                        {reg.phone}
                    </a>
                    <p className='reg-card__paragraph'>
